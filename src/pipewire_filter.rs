...
use process::FilterCallbackData;
...
impl UnconnectedFilter {
    pub fn new(loop_: &Loop, name: &str) -> Result<Self, FilterCreateError> {
        ...
        // pw_filter_new_simple takes ownership of properties. The callback data
        // has a stable heap address and outlives raw.
    ...
    pub fn add_mono_port(
        ...
        self.callback_data
            .add_port(raw, descriptor.direction, descriptor.channel.clone());
    ...
    pub fn enable_meter(&mut self, sample_rate: u32) -> Result<(), ebur128_stream::Error> {
        self.callback_data.enable_meter(sample_rate)
    }
...
impl ConnectedFilter {
    pub fn set_target_gain_db(&self, target_gain_db: f32) -> Result<(), &'static str> {
        self.filter.callback_data.set_target_gain_db(target_gain_db)
    }

    pub fn target_gain_db(&self) -> f32 {
        self.filter.callback_data.target_gain_db()
    }

    pub fn latest_meter_snapshot(&self) -> Option<MeterSnapshot> {
        self.filter.callback_data.latest_meter_snapshot()
    }
